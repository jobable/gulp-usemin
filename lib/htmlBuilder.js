module.exports = function(file, blocks, options, push, callback) {
  ...
  function parseSrcPath(filename, contents, options) {
    var versionHash = options.md5ParamKey ? md5(contents).substring(0, MD5_PARAMS_LENGTH) : null;
    var filePath = path.join(options.assetsBasePath || '', filename);
    return versionHash ? filePath + '?v=' + versionHash : filePath;
  }
      ...
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.js')
            var jsAttributes = options.jsAttributes ? options.jsAttributes : null;
            var srcPath = parseSrcPath(name.replace(path.basename(name), path.basename(file.path)), file.contents, options);

            html[i] += '<script src="' + srcPath
                + '"' + createHTMLAttributes(jsAttributes, jsCounter++) +'></script>';
          resolve();
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.css') {
            var cssAttributes = options.cssAttributes ? options.cssAttributes : null;
            var srcPath = parseSrcPath(name.replace(path.basename(name), path.basename(file.path)), file.contents, options);

            html[i] += '<link rel="stylesheet" href="' + srcPath
  ...
  Promise.all(promises).then(function() {
    ...
    pipeline(createdFile.path, [createdFile], options['html'], function(file) {
      callback(null, file);
    });
  });
};
